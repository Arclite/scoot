...
RUN cp "$(swift build --package-path /build -c release --show-bin-path)/scoot" ./
...
# Let Docker bind to port 8008
EXPOSE 8008

# Start the Hummingbird service when the image is run, default to listening on 8008 in production environment
ENTRYPOINT ["./scoot"]
